...
REFERENCE_MODEL = "sprif_full"  # 显著性对比基准
CHANCE = 0.5
...
def _per_seed_metrics(values):
    """按 seed 聚合出每个 seed 的 clean/stress/绝对下降/相对下降。"""
    by_seed = defaultdict(list)
    for r in values:
        by_seed[r["seed"]].append(r)
    max_k = max(r["intervention_count"] for r in values)
    max_f = max((r.get("intervention_fraction", 0.0) for r in values if r["intervention_count"] == max_k), default=0.0)
    per_seed = {}
    for seed, rows in by_seed.items():
        ...
        stress = _mean([r["accuracy"] for r in rows
                        ...
                        and r.get("intervention_fraction", 0.0) == max_f])
...
def _bootstrap_pvalue(ref, other, reps=2000, seed=0):
    """双尾 bootstrap 检验:ref 与 other 的 abs_drop 均值差是否显著。"""
...
def summarize(rows):
    ...
    per_model = {m: _per_seed_metrics(v) for m, v in grouped.items()}
...
def summarize_curve(rows):
    """逐 (model, fraction) 输出 accuracy/abs_drop + CI,供画 fraction-response 主图。
    fraction=0.0 视为 clean 基准点。"""
