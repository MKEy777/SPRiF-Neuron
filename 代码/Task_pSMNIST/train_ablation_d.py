"""SPRiF Ablation D - PS-MNIST with free 3x3 A matrix (unconstrained recurrence)."""
...
from model_ablation_d import PermutedMNIST, SPRiFpSMNISTNetAblationD

def get_args():
    p = argparse.ArgumentParser(description="SPRiF Ablation D: PS-MNIST free A matrix")
...
def main():
    ...
    print("Ablation D: free 3x3 A matrix (unconstrained, 9 params/neuron for A)")
    ...
    model = SPRiFpSMNISTNetAblationD(input_size=1, hidden_sizes=list(args.hidden_sizes),
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if va > best:
            best = va
    print(f"\nAblation D complete. Best test acc: {best:.2f}%")
