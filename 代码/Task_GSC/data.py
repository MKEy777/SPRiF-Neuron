...
class SpeechCommandsDataset(Dataset):
    def __init__(self, data_root, label_dct, mode, transform=None, cache_root=None, max_nb_per_class=None):
        ...
        if cache_root is not None:
            self.cache_dir = os.path.join(cache_root, mode)
        ...
        for root, _, files in os.walk(data_root):
            ...
            for filename in files:
                ...
                if (self.mode == "test" and testing_file) or (self.mode == "train" and training_file) or (
        ...
        if self.mode == "train":
            ...
            self.weights = torch.DoubleTensor([weights[lbl] for lbl in self.labels])
    ...
    def __getitem__(self, idx):
        ...
        if self.cache_dir is not None:
            cache_name = filename.replace("/", "_").replace("\\", "_").replace(".wav", ".npy")
            cache_path = os.path.join(self.cache_dir, cache_name)
            if os.path.exists(cache_path):
                item = np.load(cache_path)
            else:
...
class Pad:
    ...
    def __call__(self, wav_arr):
        wav_size = wav_arr.shape[0]
        if wav_size > self.size:
...
class RandomShift:
    ...
    def __call__(self, wav_arr):
        ...
        shifted_wav = np.roll(wav_arr, shift)
        if shift > 0:
            shifted_wav[:shift] = 0
        elif shift < 0:
            shifted_wav[shift:] = 0
...
class MelSpectrogram:
    ...
    def __call__(self, wav_arr):
        S = librosa.feature.melspectrogram(
            y=wav_arr, sr=self.sr, n_fft=self.n_fft, hop_length=self.hop_length, n_mels=self.n_mels, fmax=self.fmax, fmin=self.fmin
        )
        M = np.max(np.abs(S))
