...
from model_ablation_b import SPRiFGSCNetAblationB
...
def collect_param_stats(model) -> dict:
    ...
    for li, layer in enumerate(model.layers):
        ...
        omega = runtime["omega"].detach()
        stats[f"layer{li}/omega_mean"] = omega.mean().item()
        stats[f"layer{li}/omega_min"]  = omega.min().item()
        stats[f"layer{li}/omega_max"]  = omega.max().item()
...
def run_experiment(params, train_loader, valid_loader, device):
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        print(
            ...
            f"omega={metrics['layer0/omega_mean']:.3f}"
        )
