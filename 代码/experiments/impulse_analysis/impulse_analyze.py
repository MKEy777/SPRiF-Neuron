...
and records the slow-state trajectory. Compares with LIF exponential kernel and
visualizes the diversity of learned temporal filters in both time and frequency domain.
...
def compute_impulse_responses(
    ...
    slow_resp = torch.stack(slow_list, dim=1).squeeze(0)          # (H, T, 3)
    membrane_resp = torch.stack(membrane_list, dim=1).squeeze(0)  # (H, T)
    return slow_resp, membrane_resp


def compute_lif_kernel(tau: float, T: int) -> np.ndarray:
    """Theoretical LIF membrane decay: exp(-t/tau)."""
    t = np.arange(T)
    return np.exp(-t / tau)
...
def plot_lif_comparison(
    ...
    """Compare SPRiF impulse response with equivalent LIF kernel.

    For each task, pick 3 representative neurons (slow / medium / fast alpha).
    Plot SPRiF x_real (solid), LIF exp(-t/tau) (dashed), and oscillation
    magnitude sqrt(x1²+x2²) (dotted).
    """
    tasks = [t for t in ["pSMNIST", "GSC", "ECG"] if t in all_responses]
    if not tasks:
        return
    ...
    n_rows = len(tasks)
    n_cols = 3  # slow, medium, fast
    labels = ["Slow α (long memory)", "Medium α", "Fast α (short memory)"]
    ...
    for row_idx, task_name in enumerate(tasks):
        if 0 not in all_responses[task_name]:
            continue
        slow_resp, membrane_resp = all_responses[task_name][0]
        alphas = all_alphas[task_name][0]
        H = slow_resp.shape[0]

        # Pick 3 representative alphas
        alpha_sorted = np.sort(alphas)
        percentile_indices = [
            np.argmin(np.abs(alphas - alpha_sorted[max(0, len(alpha_sorted) // 10)])),   # slow (~p10)
            np.argmin(np.abs(alphas - np.median(alphas))),                                # medium
            np.argmin(np.abs(alphas - alpha_sorted[min(len(alpha_sorted) - 1, 9 * len(alpha_sorted) // 10)])),  # fast (~p90)
        ]

        for ci, (ni, label) in enumerate(zip(percentile_indices, labels)):
            ax = axes[row_idx, ci]
            alpha_val = alphas[ni]
            tau = float(np.clip(-1.0 / np.log(alpha_val + 1e-8), 0.1, 200.0))

            # SPRiF x_real
            ax.plot(slow_resp[ni, :, 0].numpy(), color="#1b9e77",
                    linewidth=2.0, label=r"SPRiF $x^{\mathrm{real}}$")

            # LIF equivalent
            lif = compute_lif_kernel(tau, T)
            ax.plot(lif, color="#e41a1c", linewidth=1.8, linestyle="--",
                    label=f"LIF exp(−t/{tau:.1f})")

            # Oscillation magnitude
            osc_mag = np.sqrt(
                slow_resp[ni, :, 1].numpy() ** 2 + slow_resp[ni, :, 2].numpy() ** 2
            )
            ax.plot(osc_mag, color="#377eb8", linewidth=0.8, linestyle=":",
                    alpha=0.8, label=r"$\sqrt{x_1^2+x_2^2}$")

            ax.set_title(f"{label}\nα={alpha_val:.3f}, τ={tau:.1f}", fontsize=9, fontweight="bold")
            ax.set_xlim(0, T - 1)
            if row_idx == 0:
    ...
    fig.suptitle(
        "SPRiF vs LIF Temporal Kernel Comparison",
        y=1.01, fontweight="bold",
    )
    fig.tight_layout()

    save_path = os.path.join(out_dir, "lif_comparison.png")
    fig.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {save_path}")
...
def main():
    ...
    for task_name, (task_dir, loader_fn) in LOADERS.items():
        ...
        for li, layer in enumerate(model.layers):
            ...
            print(f"    alpha: [{alphas_np.min():.4f}, {alphas_np.median():.4f}, {alphas_np.max():.4f}]")
            print(f"    tau:   [{tau_from_alpha.min():.1f}, {np.median(tau_from_alpha):.1f}, {tau_from_alpha.max():.1f}] steps")
            print(f"    omega: [{omegas_np.min():.4f}, {omegas_np.median():.4f}, {omegas_np.max():.4f}] rad")
            if li >= 1:
    ...
    plot_lif_comparison(all_responses, all_alphas, out_dir, T=T)
