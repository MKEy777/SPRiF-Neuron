...
def main():
    ...
    p.add_argument("--fraction", type=float, default=0.15); args = p.parse_args()
    df = pd.DataFrame(json.loads(Path(args.input).read_text(encoding="utf-8")))
    out = Path(args.output); out.mkdir(parents=True, exist_ok=True)
    k_curve_df = df
    if "intervention_fraction" in df.columns:
        k_curve_df = df[(df.intervention_count == 0) | (df.intervention_fraction == args.fraction)]
    for model, group in k_curve_df.groupby("model"):
